...
class ResourceBase(PolymorphicModel, PermissionLevelMixin):
    ...
    def save_thumbnail(self, filenameo, image):

        # if os.path.splitext(filenameo)[-1]==".pdf":
			# pass
			# img = Image(filename=filenameo+"[0]")
			# img.save(filename="/srv/www/geonode/geonode/static/geonode/img/tmp.png")
				
        thumb_folder = 'thumbs'
        upload_path = os.path.join(settings.MEDIA_ROOT, thumb_folder)
        if not os.path.exists(upload_path):
            os.makedirs(upload_path)
        #url = os.path.join(settings.MEDIA_URL, thumb_folder, filenameo)

        with open(os.path.join(upload_path, filenameo), 'w') as f:
            thumbnail = File(f)
            thumbnail.write(image)
        url_path = os.path.join(settings.MEDIA_URL, thumb_folder, filenameo)
